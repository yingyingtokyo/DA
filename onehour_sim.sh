...
EXP=$ar_year$ar_month$ar_date$ar_hour$ar_ens        # experiment name (output directory name)
...
if [ $looptype = "true" ];then
	CRESTSTO=$INBASE"/restart/true/restart"$ar_year$ar_month$ar_date$ar_hour"T000.bin" #restart file name
elif [ $looptype = "open" ];then
	CRESTSTO=$INBASE"/restart/open/restart"$ar_year$ar_month$ar_date$ar_hour"C"$ens_num".bin" #restart file name
else
	CRESTSTO=$INBASE"/restart/assim/restart"$ar_year$ar_month$ar_date$ar_hour"A"$ens_num".bin" #restart file name
fi
# echo $CRESTSTO
...
CVARSOUT="rivdph,sfcelv,outflw,fldare,fldfrc,storge" #"rivout,rivsto,sfcelv,outflw,storge,fldout,fldsto,daminf,damsto,flddph,fldare" # list output variable (comma separated) # Moiz
...
#--  loop 1-hour simulation from $YSTART to $YEND
YSTA=$in_year
ISP=1           ## spinup count
IYR=${YSTA}     ## current year
CYR=`printf %04d ${IYR}`
...
EYEAR=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour "year"`
EMON=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour "month"`
EDAY=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour "date"`
EHOUR=`python ${DA_dir}/src/calc_end_hour.py $in_year $in_month $in_date $in_hour "hour"`
