...
lead_str = pm.DA_leading()
dahour_str = '{:02d}'.format(pm.dahour())
#DT = 3600*(pm.dahour())
CaMa_out_dir = pm.CaMa_out() + dahour_str
CaMa_in_dir  = pm.CaMa_in()  + dahour_str
############
def main_act():
    ...
    os.system("ifort data_assim.f90 -o data_assim -qmkl")
    os.system("ifort make_restart.f90 -o make_restart -qmkl")
    ...
    for hour in range(0,hour_count): #Moiz: Day np.int64 doesn't work with timedelta
    ...
    if pm.output_er()==1:
        os.system("rm -Rf ./"+CaMa_out_dir+"/"+yyyy+"*")
        os.system("rm -Rf ./"+CaMa_in_dir+"/"+pm.input()+"/Roff_CORR/Roff__"+yyyy+"*")
...
def one_hour_loop(yyyy,mm,dd,hh):
    ...
    if pm.output_er()==1:
        ...
        if bef_dt != datetime.datetime(bef_dt.year,12,31,23): # to keep restart file in last hour of the year
            os.system("rm -Rf ./"+CaMa_out_dir+"/"+bef_yyyy+bef_mm+bef_dd+bef_hh+"*")
...
def spinup_loop(inputlist):
    ...
    #print(pm.CaMa_out(),pm.CaMa_in,pm.DA_dir)
    os.system("source "+str(pm.DA_dir())+"/src/spin_up.sh "+str(yyyy)+" "+str(loop)+" "+ens_num+" "+dir2+" "+str(cpunums)+" "+str(run_name)+" "+str(exp_dir)+" "+str(mapname)+" "+str(cal)+" "+CaMa_out_dir+" "+CaMa_in_dir)
    return 0
###########################
def one_hour_sim(inputlist):
    ...
    cal=pm.calibrate()
    DA_dir=pm.DA_dir()
    os.system("source "+pm.DA_dir()+"/src/onehour_sim.sh "+yyyy+" "+mm+" "+dd+" "+hh+" "+ens_num+" "+dir2+" "+looptype+" "+str(cpunums)+" "+str(run_name)+" "+str(exp_dir)+" "+str(mapname)+" "+str(cal)+" "+DA_dir+" "+CaMa_out_dir+" "+CaMa_in_dir+" "+lead_str) #+" "+str(DT)) #+str(corrupt)+" "
...
def copy_corrupted_sfcelv(inputlist):
    ...
    num  = inputlist[4]
    numch='%03d'%num
    fname="./"+CaMa_out_dir+"/"+yyyy+mm+dd+hh+"C"+numch+"/sfcelv"+yyyy+".bin"
    os.system("cp "+fname+" ./assim_"+(pm.runname(pm.mode()))+dahour_str+"/ens_xa/open/"+yyyy+mm+dd+hh+"_"+numch+"_xa.bin")
    return 0
...
def copy_corrupted_restart(inputlist):
    ...
    numch='%03d'%num
    fname="./"+CaMa_out_dir+"/"+yyyy+mm+dd+hh+"C"+numch+"/restart"+n_yyyy+n_mm+n_dd+n_hh+".bin"
    print('copy',fname)
    os.system("cp "+fname+" ./"+CaMa_in_dir+"/restart/open/restart"+n_yyyy+n_mm+n_dd+n_hh+"C"+numch+".bin") ## CaMa-Flood v4.1
...
def data_assim(yyyy,mm,dd,hh): # new data assimilation function (2020/05/18)
    ...
    os.system(pm.DA_dir()+"/src/data_assim "+yyyy+mm+dd+hh+" "\
    +str(pm.patch_size())+" "+str(pm.ens_mem(pm.mode()))+" "+nxtyyyymmddhh+" "+pm.CaMa_dir()\
    +" "+str(pm.thersold())+" "+exp_dir+" "+pm.DA_dir()+" "+pm.patch_dir()+" "\
    +" "+pm.obs_dir()+" "+str(pm.rho())+" "+str(pm.sigma_b())\
    +" "+str(pm.conflag())+" "+(pm.calibrate())+" "+(pm.runname(pm.mode()))+" "+" "+CaMa_out_dir+" "+CaMa_in_dir+" assim"+" "+str(pm.dahour())+" "+dahour_str)  # online DA
    os.system(pm.DA_dir()+"/src/data_assim "+yyyy+mm+dd+hh+" "\
    +str(pm.patch_size())+" "+str(pm.ens_mem(pm.mode()))+" "+nxtyyyymmddhh+" "+pm.CaMa_dir()\
    +" "+str(pm.thersold())+" "+exp_dir+" "+pm.DA_dir()+" "+pm.patch_dir()+" "\
    +" "+pm.obs_dir()+" "+str(pm.rho())+" "+str(pm.sigma_b())\
    +" "+str(pm.conflag())+" "+(pm.calibrate())+" "+(pm.runname(pm.mode()))+" "+" "+CaMa_out_dir+" "+CaMa_in_dir+" open"+" "+str(pm.dahour())+" "+dahour_str)  # offline DA
    return 0
###########################
...
def make_initial_restart(): # updated the name
    ...
    for num in np.arange(1,pm.ens_mem()+1):
        ...
        infile_opn =exp_dir+CaMa_in_dir+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin"
        infile_asm =exp_dir+CaMa_in_dir+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin"
        outfile_opn=exp_dir+pm.CaMa_out()+"/restart/open/restart"+yyyy+mm+dd+"C"+numch+".bin"
        outfile_asm=exp_dir+pm.CaMa_out()+"/restart/assim/restart"+yyyy+mm+dd+"A"+numch+".bin"
        inputlist.append([infile_opn,infile_asm,outfile_opn,outfile_asm])
        os.makedirs("./"+CaMa_out_dir+'/'+spinup_open,exist_ok=True)
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm_restart+dd+".bin  ./"+CaMa_out_dir+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin")
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin ./"+CaMa_in_dir+"/restart/open/restart"+yyyy+mm+dd+hh+"C"+numch+".bin")
        os.system("cp ./"+pm.CaMa_out()+"/"+spinup_open+"/restart"+yyyy+mm+dd+".bin ./"+CaMa_in_dir+"/restart/assim/restart"+yyyy+mm+dd+hh+"A"+numch+".bin")
    return 0
###########################
    ...
    for looptype in listCA: # update on 2023/05/30
        ...
        for num in np.arange(1,pm.ens_mem()+1):
            ...
            for var in ["rivdph","sfcelv","outflw","storge"]: 
                shutil.copy("./"+CaMa_out_dir+"/"+yyyy+mm+dd+hh+CA+numch+"/"+var+yyyy+".bin","./assim_"+(pm.runname(pm.mode()))+dahour_str+"/"+var+"/"+looptype+"/"+var+yyyy+mm+dd+hh+"_"+numch+".bin")
...
def make_restart(inputlist):
    ...
    bef_y=calc_odb(yyyy,mm,dd,hh,"year")
    bef_m=calc_odb(yyyy,mm,dd,hh,"month")
    bef_d=calc_odb(yyyy,mm,dd,hh,"date")
    bef_h=calc_odb(yyyy,mm,dd,hh,"hour")
    ...
    exp_dir="./" #pm.DA_dir()+"/out/"+pm.experiment()
    os.system(pm.DA_dir()+"/src/make_restart "+yyyy+mm+dd+hh+" "+yyyy_b+mm_b+dd_b+hh_b+" "+yyyy_n+mm_n+dd_n+hh_n+" "+loop+" "+pm.CaMa_dir()+" "+pm.mapname()+" "+numch+" "+exp_dir+" "+CaMa_out_dir+" "+CaMa_in_dir+" "+(pm.runname(pm.mode()))+" "+dahour_str)
...
def prepare_input():
    ...
    return 0
###########################
def calc_odb(year,month,date,hour,obj):
    year=int(year)
    month=int(month)
    date=int(date)
    hour=int(hour)
    #calc bef hour
    if (hour-pm.dahour()>0) | (hour-pm.dahour()==0):
        bef_h = hour-pm.dahour()
        bef_d = date
        bef_m = month
        bef_y = year
    else:
        bef_h = hour-pm.dahour()+24
        bef_d = date-1
        bef_m = month
        bef_y = year
...
def read_sim_data(numch):
    ...
    mm="%02d"%(month)
    dd="%02d"%(date)
    file = "/work/a06/yingying/camada/HydroDA/src/"+CaMa_out_dir+'/'+yyyy+mm+dd+"C"+numch+"/2019-sp1/o_rivdph2019.nc"
    print(file)
    return file
def calc_mean():
